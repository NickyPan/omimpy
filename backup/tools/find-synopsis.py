...
latest_entries = json.load(open('./omim_latest-201805.json'))
...
def search_list(phenos):
    for item in phenos:
        if isinstance(item, str) and key_str in item:
            print (item)
            return True
        elif isinstance(item, list) and key_str in item:
            isSearched2 = search_list(item)
            if isSearched2:
                return True
        elif isinstance(item, dict):
            for item3 in item.values():
                isSearched = search_list(item3)
                if isSearched:
                    return True
        else:
            print (item)
...
for item in latest_entries:
    if 'clinical' in item.keys() and item['omim_type'] == '#':
        if 'Eyes' in item['clinical'].keys():
            for item3 in item['clinical']['Eyes']:
                if key_str in item3 and int(item['omim_num']) not in key_arr:
                    key_arr.append(int(item['omim_num']))
        if 'HeadAndNeck' in item['clinical'].keys():
            for item2 in item['clinical']['HeadAndNeck']:
                if 'Eyes' in item2.keys():
                    for item3 in item2['Eyes']:
                        if key_str in item3 and int(item['omim_num']) not in key_arr:
                            key_arr.append(int(item['omim_num']))
        # clinicalList = list(item['clinical'].values())
        # for item2 in clinicalList:
        #     if 'Eyes' in item2.keys():
        #         for item3 in item2['Eyes']:
        #             if key_str in item3 and int(item['omim_num']) not in key_arr:
        #                 key_arr.append(int(item['omim_num']))
            # isSearched = search_list(item2)
            # if isSearched and int(item['omim_num']) not in key_arr:
            #     key_arr.append(int(item['omim_num']))
                # print (item['omim_num'], string)
        # if isinstance(clinicalList, list) and key_arr in :
        #     print (item['clinical'])
...
print(key_arr)
print(len(latest_entries), len(key_arr))
